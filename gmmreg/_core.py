...
'''
Code for Gaussian Mixture Model point set registration adapted from "Robust Point Set Registration Using Gaussian Mixture Models", Jian and Vemuri 2011
Implementation of standard GMM registration here: https://github.com/bing-jian/gmmreg
Method for computing the gradient of scale adjusted L2 distance adapted from Jeffery Nguyen's code for "Automatically tracking neurons in a moving and deforming brain" 2017
Matlab implementation that the L2 distance calculation is based off of found here: https://github.com/leiferlab/NeRVEclustering
'''
...
def L2_distance(model, scene, m_info, s_info, size_scale, intensity_scale):
    ...
    peaksJoined = np.concatenate((peaksA, peaksB))
    peaksMat = np.asarray(list(map(lambda x: x * peaksJoined, peaksA)))

    varA = np.square(a_surf)
    varB = np.square(b_surf)
    varJoined = np.concatenate((varA, varB))
    varMat = np.asarray(list(map(lambda x: x + varJoined, varA)))

    joined = np.concatenate((model, scene))
    reshapeJoined = joined.reshape((len(joined), len(joined[0]), 1))
    reshapeA = model.reshape((len(model), len(model[0]), 1))

    Bp = np.transpose(reshapeJoined, (2, 0, 1))
    A1 = np.transpose(reshapeA, (0, 2, 1))
    dmatAll = np.asarray(list(map(lambda x: x - Bp, A1)))

    dmatAll = A1 - Bp
    dmat2 = np.sum(np.square(dmatAll),axis=2);

    expMat= np.multiply(peaksMat, np.exp(-2 * np.divide(dmat2, varMat)))
    expMatSelf=expMat[:,range(len(model))]
    expMat1=expMat[:,range(len(model), len(joined))]

    fself = np.sum(expMatSelf)
    f2 = np.sum(expMat1)

    expMat2= -np.multiply(np.divide(4, varMat), expMat)
    fmat = np.multiply(expMat2.reshape(len(expMat2), len(expMat2[0]), 1), dmatAll)
    gself = np.sum(fmat[:, range(len(model))], axis=1).reshape((len(model), 2))
    g2 = np.sum(fmat[:, range(len(model), len(joined))], axis=1).reshape((len(model), 2))

    f =  fself - 2*f2
    g = 2*array(gself) - 2*array(g2)
    return f,g

def original_L2_distance(model, scene, m_info, s_info, scale):
...
def OLD_L2_distance(model, scene, m_info, s_info, scale):
    """
    ...
    m = matlab.double(model.tolist())
    s = matlab.double(scene.tolist())
    mi = matlab.double(m_info.tolist())
    si = matlab.double(s_info.tolist())

    [f1, g1, f2, g2] = eng.gaussOverlapDouble(m, s, mi, si, nargout=4)
